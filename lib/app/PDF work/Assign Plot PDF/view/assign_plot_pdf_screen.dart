...
class AssignPlotPdfScreen extends GetView<AssignPlotPdfController> {
  AssignPlotPdfScreen({super.key});
  ...
  Widget build(BuildContext context) {
   return Scaffold(
  appBar: AppBar(
    title: const Text("Allotment Letter"),
    centerTitle: true,
    elevation: 0, // Remove app bar shadow for a cleaner look
  ),
  body: FutureBuilder<List<AssignPlotPdfModel>>(
    future: controller.getAssignPlot(),
    builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        final List<AssignPlotPdfModel> plotList = snapshot.data ?? [];
        return Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
          child: ListView.builder(
            itemCount: plotList.length,
            itemBuilder: (context, index) {
              final pdfmodel = plotList[index];
              return Card(
...
//   _viewPDf(AssignPlotPdfModel pdfList) {
//     return Scaffold(
//         appBar: AppBar(
//           title: const Text("Alltment Letter"),
//         ),
//         body: PDFView(
//           filePath: pdfList.pdfLink,
//         ));
//   }
}
